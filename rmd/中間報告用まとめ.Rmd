...
```{r here}
customer_ %>% 
  mutate(first_store_name = factor(customer_$first_store, labels = c(store$store_name[13],store$store_name[1:12]) %>% trimws())) %>% 
  filter(first_store != 0) %>%
  filter(sex =="女性" | sex =="男性") %>% 
  group_by(first_store_name) %>% 
  summarise(Mean = mean(visit_interval,na.rm = TRUE),
            Median = median(visit_interval,na.rm = TRUE)) -> lab
labs <- data.frame(first_store_name = c(store$store_name[1:12])%>%
                     trimws(),
...
customer_ %>% 
  ...
  geom_text(data = labs, aes(x = 250,y = 400,label = pval),size = 3.25) +
